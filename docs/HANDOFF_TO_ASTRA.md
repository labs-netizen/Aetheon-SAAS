...
> **Repository Commit Checkpoint**: `fix(v1): complete pre-astra functional blocker pass, real persistence and verification`  
> **Engineering Tag**: `antigravity-functional-v1.1`  
> **Date**: September 2026  
> **Verification Status**: Real Local Supabase (PostgreSQL 17.6 + Auth + Storage + Kong) Active and Healthy; All 82 Automated Test Specs Passing (75 Vitest across 11 test files + 7 Pytest across 1 file = 100% Pass Rate). All 14 Playwright E2E Tests Passing across 3 suites. All 39 Next.js App Routes Compiling Cleanly.
...
- **Live Local Supabase Architecture**: Running in Docker on Windows (ports mapped to 15431–15437 to bypass Hyper-V exclusions). PostgreSQL 17.6 database is fully migrated with 11 migrations (`20260907000001` through `20260907000011_pre_astra_blockers.sql`) and seeded with tenant organizations, sites, site access grants, discom tariffs, regulatory records, and atomic transactional RPCs.
...
- **Real Transactional Ingestion & Server Checksums**:
  - Ingestion commit endpoint (`/api/ingestion/commit`) accepts raw multipart CSV, computes SHA-256 on actual file content, enforces file duplicate rejection (HTTP 409), transactionally executes via PostgreSQL RPC `commit_ingestion_transaction`, and transitions site status accurately (`CALIBRATING` on first valid data; `ACTIVE` only when full history and freshness prerequisites are met).
...
|---|---|---|---|---|---|---|---|
| **Unit & Contract Tests** | Vitest | Node.js | 8 files | 40 tests | 40 | 0 | **PASSED** |
...
| **Adversarial API Tests** | Vitest | Node.js / Next.js Handlers | 1 file | 20 tests | 20 | 0 | **PASSED** |
...
| **TOTAL AUTOMATED VERIFICATION** | | | **15 test files** | **96 tests** | **96 passed** | **0 failed** | **100% PASS RATE** |
...
- **Implemented**: Explicit modes (`MOCK_DEVELOPMENT`, `RAZORPAY_TEST`, `RAZORPAY_LIVE`), cryptographic HMAC-SHA256 signature verification, pre-inserted checkout session mapping (`billing_checkout_sessions`), and atomic processing in `process_razorpay_webhook_atomic`.
...
- **Files**: [`src/app/api/ingestion/commit/route.ts`](file:///d:/Consultancy%20Project/Aetheon-SAAS/src/app/api/ingestion/commit/route.ts), [`supabase/migrations/20260907000011_pre_astra_blockers.sql`](file:///d:/Consultancy%20Project/Aetheon-SAAS/supabase/migrations/20260907000011_pre_astra_blockers.sql)
- **Implemented**: Multipart CSV upload, server SHA-256 calculation, duplicate rejection (409), contiguous 96-block validation, and transactional commitment via `commit_ingestion_transaction`. Site status moves to `CALIBRATING` on first valid data; moves to `ACTIVE` only when full calibration history and freshness are satisfied.
