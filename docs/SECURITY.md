...
- **Row Level Security (RLS)**: Enforced directly at the PostgreSQL layer in the final 22-migration schema. All tenant tables (`sites`, `interval_data_96`, `subscriptions`, `alerts`, `audit_logs`, `compliance_obligations`, `report_records`) contain an `organisation_id` foreign key.
