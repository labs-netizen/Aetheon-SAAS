...
- **Server-Only Operational Writes**: Tables `forecast_runs`, `grid_forecast_blocks`, `bess_signal_runs`, `dsm_incidents`, `interval_data_96`, `ingestion_runs`, `site_activation_history`, and `report_records` reject all direct INSERT/UPDATE/DELETE from client sessions (`auth.role() = 'service_role'` required).
...
## 6. Controls Requiring Production Configuration / Audit
