...
graph TD
    ...
    API -->|Atomic Transactions| RPCS[PostgreSQL RPCs: Migration 12 & 13]
...
├── supabase/
│   ├── migrations/               # 13 versioned SQL DDL/RLS migrations
│   └── seed.sql                  # Canonical seed data (discom tariffs, demo org, CEA factors)
├── tests/
│   ├── unit/                     # Fast isolated unit tests (csv, currency, quality, etc.)
│   ├── integration/              # Live DB tests (supabase_rls, audit_chaining, adversarial_api)
│   └── e2e/                      # Playwright suites (demo_smoke, registration, persistence)
...
| **DSM Monitor** | `src/app/dsm/page.tsx` | `src/app/api/dsm/route.ts` | `dsm_incidents` | `acknowledge_dsm_incident_atomic()` |
...
| **Billing** | `src/app/settings/page.tsx` | `src/app/api/billing/checkout/route.ts` | `subscriptions`, `invoices`, `billing_checkout_sessions` | `src/app/api/webhooks/razorpay/route.ts` |
