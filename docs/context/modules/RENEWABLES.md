...
- **External Service**: Python FastAPI microservice (`POST /renewables/reconcile`).
...
- **Alerts**: Dispatches `RENEWABLE_UNDERPERFORMANCE` when measured solar generation is $< 80\%$ of clear-sky modelled output.
...
- **Tests**: `services/analytics/tests/test_analytics.py` (`test_renewable_reconciliation`), `tests/integration/adversarial_api.test.ts` (Test 19).
