...
describe('BESS sizing evidence discovery',()=>{
  it('keeps structurally valid price days provisional until canonical forecast readiness is evaluated',()=>{const target=date(50);
    ...
    expect(catalog.find(day=>day.date===target)).toMatchObject({eligible:false,sizing_ready:false,price_verified:true,forecast_ready:false,
      price_blocks:96,price_verification:'VERIFIED',reason_if_ineligible:'FORECAST_READINESS_NOT_EVALUATED'});
    expect(catalog.find(day=>day.date===date(49))).toMatchObject({eligible:false,price_blocks:95,reason_if_ineligible:'IEX_PRICE_INCOMPLETE'});
    expect(catalog.find(day=>day.date===date(48))).toMatchObject({eligible:false,price_verification:'UNVERIFIED',reason_if_ineligible:'IEX_PRICE_UNVERIFIED'});
  });
  it('excludes insufficient history and missing profiles',()=>{expect(buildBessSizingEvidenceCatalog(history.slice(0,10),prices(date(10)),true,'2027-01-01')[0].reason_if_ineligible).toBe('INSUFFICIENT_LOAD_HISTORY');
